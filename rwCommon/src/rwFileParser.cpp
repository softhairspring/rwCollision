...
*/
#include "config.h" //for unit testing
...
#include "rwFileParser.h"
#include "rwLogLevel.h"

rwFileParser::rwFileParser(string name, string parentDebugInfo) {
	debugInfo = parentDebugInfo + ", " + name;
}

rwFileParser::rwFileParser(string configFile, int &error) {
	...
	while (fgets(buff, 1024, in)) {
		...
		if ( (line.length() > 2) && (line[0] != '#') && (line.find(')') == string::npos) ) {
			...
			if (value == "(") {
				...
				rwFileParser* newGroup = new rwFileParser(name, debugInfo);
				groupStack.front()->groups[name] = newGroup;
				groupStack.push_front(newGroup);
			} else {
				for (list<rwFileParser*>::reverse_iterator i = groupStack.rbegin(); i != groupStack.rend(); ++i) {
...
rwFileParser::rwFileParser(string configFile, char** envp) {
	while (*envp) {
...
rwFileParser::~rwFileParser() {
	for (map<string, rwFileParser*>::iterator i = groups.begin(); i != groups.end(); ++i) {
		delete i->second;
	}
}

void rwFileParser::add(string name, string value) {
	symbols[name] = value;
}

void rwFileParser::split(string in, string& left, string& right, char c) {
...
void rwFileParser::trim(string& s) {
...
void rwFileParser::symbolExpand(string& s) {
	symbolExpand(symbols, s);
}

void rwFileParser::envSymbolExpand(string& s) {
	symbolExpand(envSymbols, s);
}

void rwFileParser::symbolExpand(map<string, string>& symbols, string& s) {
	bool expanded;
	do {
		expanded = false;
...
string rwFileParser::pString(string name) {
	map<string, string>::iterator i = symbols.find(name);
	if (i == symbols.end()) {
		logError(cout << "access of missing property '" << name << "' (" << debugInfo << ")" << endl);
		bool configINIaccessofMissingProperty=false;
		assert(configINIaccessofMissingProperty);
		return string("");
	}
	return i->second;
}

bool rwFileParser::pBool(string name) {
...
double rwFileParser::pDouble(string name) {
	string val = pString(name);

	return atof(val.c_str());
}

int rwFileParser::pInt(string name) {
	string val = pString(name);
