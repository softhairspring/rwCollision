...
void ProfileInit( void )
{
  ...
  textBox = new TextBox(0, 640, 0, 240);
...
void ProfileEnd( char* name )
{
   ...
   if(ISLOG){
	   rwTimer.stop();
	   Logger.Write( rwTimer.getFrameInMiliSeconds() );
   }
...
void StoreProfileInHistory( char* name, float percent )
{
   ...
   while( i < NUM_PROFILE_SAMPLES && g_history[i].bValid == true ) {
      ...
      {  //Found the sample
		 ...
		 if(g_history[i].fAve <  0){
			g_history[i].fAve =   g_history[i].fMin;
		 }
...
void StartTimeLoggingToFile()
{
		ISLOG=true;
}
void StopTimeLoggingToFile()
{
		ISLOG=false;
}